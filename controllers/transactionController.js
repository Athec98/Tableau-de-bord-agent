...
exports.createDeposit = async (req, res) => {
  const { agentId, distributorId, montant } = req.body;

  try {
    // Vérifier si l'agent et le distributeur existent
    const agent = await User.findById(agentId);
    const distributor = await User.findById(distributorId);

    if (!agent || agent.role !== "agent") {
      return res.status(404).json({ msg: "Agent non trouvé ou rôle incorrect" });
    }
...
exports.getTransactionsHistory = async (req, res) => {
  try {
    const transactions = await Transaction.find()
      .populate("agentId", "nom prenom email numeroCompte")
      .populate("distributorId", "nom prenom email numeroCompte")
